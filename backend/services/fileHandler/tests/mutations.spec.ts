...
describe('[Mutations]', () => {
    ...
    describe('move', () => {
        it('should return moved files', async () => {
            const { data } = await client.query({query: moveObjectQuery, variables: {bucket: "documnets", fileNames: ["public/filename.png"], destination: "public/folder/"}});
            expect(dataSources.filesAPI.moveObject).to.have.been.called();
        });
    });
    describe('remove', () => {
        it('should return a removed file', async () => {
            const { data } = await client.query({query: removeObjectQuery, variables: {bucket: "documnets", fileNames: ["public/filename.png"]}});
