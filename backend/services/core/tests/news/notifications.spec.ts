...
const insertNotifications = async () => {
  authors = await knex('authors').insert(members.map((member) => ({
    member_id: member.id,
  }))).returning('*');
  await knex<SQLNotification>('notifications').insert([
    ...(authors.slice(1).map((author, index) => ({
      link: '/news/article/testarticle', // not a valid link
      type: NotificationSettingType.LIKE,
      title: 'Testartikel',
      message: `${getFullName(members[index + 1])} har gillat din nyhet`,
      member_id: members[0].id,
      from_author_id: author.id,
      created_at: randomDateWithinLastWeek(),
    }))),
    ...(authors.slice(2).map((author, index) => ({
    ...
    ...(authors.slice(1).map((author, index) => ({
      ...
      title: `${getFullName(members[index + 1])} har kommenterat på Testartikel`,
      message: `Detta är min kommentar // Mvh, ${members[index + 1].first_name} `,
      member_id: members[0].id,
      from_author_id: author.id,
      created_at: randomDateWithinLastWeek(),
    }))),
    ...
    ...(authors.slice(1, 4).map((author, index) => ({
      ...
      title: `${getFullName(members[index + 1])} har nämnt dig i Testartikel`,
      message: `Detta är min kommentar @${members[index + 1].first_name} ${members[index + 1].last_name} // Mvh, ${members[index + 1].first_name} `,
      member_id: members[0].id,
      from_author_id: author.id,
      created_at: randomDateWithinLastWeek(),
    }))),
    ...(authors.slice(1).map((author, index) => ({
      ...
      title: 'An event',
      message: `${getFullName(members[index + 1])} kommer`,
      member_id: members[0].id,
      from_author_id: author.id,
      created_at: randomDateWithinLastWeek(),
    }))), {
    ...
    ...(authors.slice(1).map((author, index) => ({
      ...
      title: 'PING!',
      message: `${getFullName(members[index + 1])} har pingat dig`,
      member_id: members[0].id,
      from_author_id: author.id,
      created_at: randomDateWithinLastWeek(),
    }))),
...
describe('[NotificationsAPI]', () => {
  ...
  describe('[getMyNotifications]', () => {
    ...
    it('returns merged tokens for member', async () => {
      ...
      expect(likeNotification?.authors?.length).to.be.equal(members.length - 1);
      ...
      expect(bookingRequestNotification?.authors?.length).to.be.equal(1);
    });
    ...
    it('only returns the current user\'s notifications', async () => {
      ...
      expect(notification.authors.length).to.equal(members.length - 2);
